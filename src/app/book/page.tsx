      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-[var(--text-3)] mb-6">
            We've received your booking request and will confirm within 2 hours. Check your email at <strong className="text-[var(--text-2)]">{form.email}</strong>.
          </p>
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-8">
              ...
              {tours.map((t) => (
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded-full border shrink-0 mt-0.5 flex items-center justify-center ${
                    form.tour === t.slug ? 'border-[var(--gold)] bg-[var(--gold)]' : 'border-[var(--text-4)]'
                  }`}>
                    {form.tour === t.slug && <div className="w-2 h-2 rounded-full bg-[var(--bg-base)]" />}
                  </div>
                  <div>
                    <div className="text-[var(--text-1)] text-sm font-semibold">{t.title}</div>
                    <div className="text-[var(--text-4)] text-xs mt-0.5">{t.duration} · {formatPrice(t.price, t.currency)}/person</div>
                  </div>
                </label>
              ))}
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 flex items-center gap-2">
                  <CalendarDays className="w-4 h-4 text-[var(--gold)]" /> Preferred Date
                </label>
                <input
                  type="date"
                  required
              ...
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 flex items-center gap-2">
                  <Users className="w-4 h-4 text-[var(--gold)]" /> Number of People
                </label>
                <select
                  value={form.guests}
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 block">Full Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 block">Email</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 block">Phone / WhatsApp</label>
                <input
                  type="tel"
              ...
              <div>
                <label className="text-[var(--text-2)] text-sm font-medium mb-2 flex items-center gap-2">
                  <Globe className="w-4 h-4 text-[var(--gold)]" /> Preferred Language
                </label>
                <select
                  value={form.language}
            ...
            <div>
              <label className="text-[var(--text-2)] text-sm font-medium mb-2 flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-[var(--gold)]" /> Special Requests (optional)
              </label>
              <textarea
                rows={3}
              ...
              {selectedTour ? (
                <>
                  <div className="mb-4 pb-4 border-b border-[var(--border-sm)]">
                    <div className="text-[var(--text-2)] font-semibold mb-1">{selectedTour.title}</div>
                  ...
                  <div className="space-y-2 mb-4 pb-4 border-b border-[var(--border-sm)] text-sm">
                    <div className="flex justify-between">
                      <span className="text-[var(--text-4)]">Price per person</span>
                      <span className="text-[var(--text-2)]">{formatPrice(selectedTour.price, selectedTour.currency)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[var(--text-4)]">× {form.guests} people</span>
                      <span className="text-[var(--text-2)]">{form.guests}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-baseline mb-6">
                    <span className="text-[var(--text-2)] font-semibold">Estimated Total</span>
                    <span className="text-[var(--gold)] font-bold text-2xl">{formatPrice(totalPrice, selectedTour.currency)}</span>
                  </div>
                </>
              ) : (
                <p className="text-[var(--text-4)] text-sm mb-6">Select a tour to see pricing.</p>
              )}
        ...
        <section className="py-14 bg-[var(--bg-surface)] border-b border-[var(--border-sm)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <span className="text-[var(--gold)] text-sm uppercase tracking-[0.2em] font-medium">Reserve Your Spot</span>
